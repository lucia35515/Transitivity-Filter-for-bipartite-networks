...
#define DATASET_NAME "sample_size0"

#define realisations    4
...
printf("Rewiring\n");
    ...
    int half_features = (N_features - N_n) / 2;
...
for(int re = 0; re < realisations; re++){
    ...
    for(i = N_n; i < N_features; i++){
        ...
        for(i = N_n; i < N_n + half_features; i++) {fprintf(fp2,"%d\t%d\t%f\n",1, features[i].degree, features[i].clustering_rewiring);}
        for(i = N_n + half_features; i < N_features; i++) {fprintf(fp2,"%d\t%d\t%f\n",2, features[i].degree, features[i].clustering_rewiring);}
